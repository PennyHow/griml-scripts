...
for f in list(glob.glob(indir+'*.csv')):
    ...
    df['ST_dt'] = [datetime.datetime.fromtimestamp(d/1000).strftime("%Y-%m-%d %H:%M") for d in df["system:time_start"]]
    df = df[['lake_id', 'ST', 'ST_count_int', 'ST_max_int', 'ST_min_int', 
             'ST_stddev_int', 'ST_dt', 'LC', 'year']]
    dfs.append(df)
...
for f in sorted(list(glob.glob(gdf_files))):
    ...
    print(str(Path(f).name))
    print(len(gdf))
    df_st_y = df_st.loc[(df_st['year'] == year)]
    
    df_st_mean = df_st_y[['lake_id', 'ST', 'ST_stddev_int']] 
    ...
    df_st_scenes = df_st_y[['lake_id', 'ST_count_int']] 
    df_st_scenes = df_st_scenes.groupby('lake_id').count()

    df_st_time = df_st_y.groupby(['lake_id'])['ST_dt'].apply(', '.join)

    gdf = gdf.merge(df_st_mean, on='lake_id', how='left')
    gdf = gdf.merge(df_st_max, on='lake_id', how='left')
    gdf = gdf.merge(df_st_min, on='lake_id', how='left')
    gdf = gdf.merge(df_st_scenes, on='lake_id', how='left')
    gdf = gdf.merge(df_st_time, on='lake_id', how='left')

    gdf['temp_aver']=gdf['ST']
    gdf['temp_max']=gdf['ST_max_int']
    gdf['temp_min']=gdf['ST_min_int']    
    gdf['temp_stdev']=gdf['ST_stddev_int']
    gdf['temp_count']=gdf['ST_count_int']
    gdf['temp_date']=gdf['ST_dt']
    gdf.loc[gdf['temp_aver'].isnull(), 'temp_count'] = np.nan
    gdf.loc[gdf['temp_aver'].isnull(), 'temp_date'] = np.nan

    gdf = gdf[['geometry', 'lake_id', 'margin', 'region', 'lake_name', 
               'area_sqkm', 'length_km', 'temp_aver', 'temp_min', 'temp_max', 
               'temp_stdev', 'temp_count', 'temp_date', 'method', 'source',
               'all_src', 'num_src', 'certainty', 'start_date', 'end_date',
               'verified', 'verif_by', 'edited', 'edited_by']]
    print(len(gdf))
    ...
    gdf.to_file(str(year)+'0101-ESA-GRIML-IML-fv1.gpkg')
